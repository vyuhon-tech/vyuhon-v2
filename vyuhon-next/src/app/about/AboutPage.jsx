...
const TEAM = [
  { initials: 'DD', name: 'Dinesh Doddaga', role: 'Co-Founder & CEO', image: '/team-dinesh-v3.jpeg', gradA: '#8B5CF6', gradB: '#7C3AED' },
  { initials: 'SD', name: 'Subhash Doddaga', role: 'Co-Founder & CTO', gradA: '#10B981', gradB: '#059669' },
  { initials: 'JG', name: 'Jagadeesh S', role: 'CPPO', image: '/team-jagadeesh.png', gradA: '#3B82F6', gradB: '#2563EB' },
]
  ...
  return (
    <>
      <div style={{ paddingTop: '68px' }}>
        <section ref={heroRef} className="relative section overflow-hidden" style={{ background: 'linear-gradient(165deg, #0D0C18 0%, #0A0A0F 55%, #0B1018 100%)', minHeight: 380 }}>
          <div className="absolute pointer-events-none rounded-full blur-[160px]" style={{ width:600, height:500, top:'-10%', left:'-5%', background:'rgba(139,92,246,0.13)' }} />
          ...
          <Container className="relative z-10">
            <div className="max-w-[700px]">
              ...
              <h1 data-hero-word className="display-lg text-white mb-6">Built by engineers.<br />Designed to ship.</h1>
              <blockquote data-hero-sub className="pl-5 font-heading font-light italic leading-[1.78] max-w-[580px]" style={{ color:'rgba(255,255,255,0.48)', borderLeft:'2px solid rgba(139,92,246,0.45)', fontSize:'1.05rem' }}>
                "Most enterprises fail at AI not because they lack data or budget — but because they lack the right engineering DNA. We built Vyuhon to change that."
              </blockquote>
            </div>
          </Container>
        </section>
          ...
          <Container>
            <div className="grid grid-cols-1 lg:grid-cols-[1.05fr_0.95fr] gap-12 items-start">
              <div>
                ...
                <p className="s1 body-lg mb-4">Vyuhon was founded by a team who spent years in the gap between AI research and real deployment — seeing incredible models fail in production for preventable reasons.</p>
                <p className="s1 body-md mb-4">We are an AI-first engineering studio: small enough to be decisive, deep enough to tackle the hardest problems. We believe the best AI systems are designed with a product mindset.</p>
              ...
              <div className="s1 grid grid-cols-2 gap-3 mt-8 lg:mt-10">
                {[
                  ...
                  <div key={i} className="p-6 rounded-[var(--r-xl)] border transition-all duration-250 hover:-translate-y-1 relative overflow-hidden cursor-default" style={{ background:'var(--surface)', borderColor:'var(--border)' }}
                    ...
                    <div className="font-heading text-[1.5rem] font-extrabold mb-1" style={item.grad ? { background:item.grad, WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent', backgroundClip:'text' } : { color:'var(--ink)' }}>{item.val}</div>
                    <div className="text-[0.74rem] font-medium" style={{ color:'var(--ink-soft)' }}>{item.label}</div>
                  </div>
                ))}
                <div className="col-span-2 p-5 rounded-[var(--r-xl)] border flex items-center gap-3" style={{ background:'var(--surface)', borderColor:'var(--border)' }}>
        ...
        <section className="section overflow-hidden" style={{ background:'var(--surface)' }} ref={s2}>
          <Container>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {VALUES.map((v,i) => (
                <div key={i} className="s2 group p-7 bg-white border rounded-[var(--r-xl)] transition-all duration-250 cursor-default relative overflow-hidden" style={{ borderColor:'var(--border)' }}
                  ...
                  <div className="absolute left-0 top-8 bottom-8 w-[2px] rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-250" style={{ background:v.color }} />
        ...
        <section className="section bg-white" ref={s3}>
          <Container>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 max-w-[960px] mx-auto gap-x-8 gap-y-10 justify-items-center">
              {TEAM.map((t,i) => (
                <div key={i} className="s3 group relative cursor-default w-full"
                  onMouseEnter={e=>{const n=e.currentTarget.querySelector('h3');if(n)n.style.color=t.gradA;}}
                  onMouseLeave={e=>{const n=e.currentTarget.querySelector('h3');if(n)n.style.color='';}}>
                  <div className="w-full aspect-[4/3.2] rounded-2xl mb-5 flex items-center justify-center relative overflow-hidden" style={{ background:'#EFF4FF' }}>
                    {t.image ? (
                      <img src={t.image} alt={t.name} loading="lazy" className="w-full h-full object-cover" />
                    ) : (
                      <div className="font-heading text-[3.5rem] font-extrabold" style={{ background:`linear-gradient(135deg,${t.gradA},${t.gradB})`, WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent', backgroundClip:'text' }}>{t.initials}</div>
                    )}
                  </div>
      ...
      <CTABanner title="Stay in touch" subtitle="We're always excited to connect with talented engineers, researchers, and builders." primaryLabel="Get in Touch →" primaryTo="/contact" />
    </>
  )
}
