  ...
  return (
    <Link href={`/insights/${p.slug}`}
      ...
      <div className="p-7 md:p-9 flex-1">
        ...
        <h3 className="font-heading font-bold tracking-[-0.015em] leading-[1.3] mb-3 transition-colors duration-200 group-hover:text-brand-purple" style={{ fontSize:'1.28rem', color:'var(--ink)' }}>{p.title}</h3>
  ...
  return (
    <Link href={`/insights/${p.slug}`}
      ...
      <div className="px-6 pb-6 pt-2 flex-1 flex flex-col">
        ...
        <h4 className="font-heading font-bold tracking-[-0.01em] leading-[1.35] mb-2 flex-1 line-clamp-3 transition-colors duration-200 group-hover:text-brand-purple" style={{ fontSize:'0.96rem', color:'var(--ink)' }}>{p.title}</h4>
        ...
        <div className="flex items-center justify-between border-t pt-4" style={{ borderColor:'var(--border)' }}>
          ...
          <span className="flex items-center gap-1 text-[0.79rem] font-semibold opacity-0 group-hover:opacity-100 transition-opacity" style={{ color:p.color }}>Read <ArrowUpRight size={12}/></span>
        </div>
      </div>
    </Link>
  )
...
export default function InsightsPage() {
  ...
  return (
    <div style={{ paddingTop:'68px' }}>
      <section ref={heroRef} className="relative py-20 overflow-hidden" style={{ background:'linear-gradient(160deg, #09090F 0%, #0A0C18 55%, #08100F 100%)', minHeight:380 }}>
        <div className="absolute inset-0 pointer-events-none" style={{ background:'radial-gradient(ellipse 65% 55% at 70% 30%, rgba(59,130,246,0.18) 0%, rgba(139,92,246,0.08) 45%, transparent 70%)' }} />
        <div className="absolute pointer-events-none rounded-full blur-[90px]" style={{ width:400, height:320, bottom:'-8%', left:'-3%', background:'rgba(16,185,129,0.1)' }} />
        ...
        <Container className="relative z-10">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
            <div>
              ...
              <h1 data-hero-word className="font-heading font-extrabold tracking-[-0.033em] text-white leading-[1.04]" style={{ fontSize:'clamp(2.2rem, 4.5vw, 4rem)' }}>
                Thinking at the frontier
              </h1>
            </div>
            <p data-hero-sub className="max-w-[360px] text-[1rem] leading-[1.82] md:pb-2" style={{ color:'rgba(255,255,255,0.46)' }}>
      ...
      <section className="section-sm pb-20 bg-white" ref={g}>
        <Container>
          ...
          <div className="mt-10 p-6 rounded-[var(--r-xl)] border flex flex-col sm:flex-row items-start sm:items-center gap-5" style={{ background:'var(--surface)', borderColor:'var(--border)' }}>
            ...
            <div className="flex gap-2 w-full sm:w-auto">
              ...
              <button className="px-5 py-2.5 rounded-xl text-sm font-semibold text-white whitespace-nowrap" style={{ background:'linear-gradient(135deg,#8B5CF6,#7C3AED)', boxShadow:'var(--shadow-p)' }}>Subscribe</button>
            </div>
          </div>
        </Container>
      </section>
