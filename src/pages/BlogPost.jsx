...
function PostBanner({ color, bg, cat }) {
  ...
  return (
    <div className="relative w-full overflow-hidden rounded-[var(--r-2xl)]"
      style={{ height: 320, background: 'linear-gradient(160deg, #070610 0%, #09091A 60%, #0A0F18 100%)' }}>
      {/* Grid */}
      ...
      <div className="absolute pointer-events-none"
        style={{ width:560, height:420, top:'-20%', left:'50%', transform:'translateX(-50%)',
          background:`radial-gradient(ellipse, ${c.a} 0%, ${c.b} 35%, transparent 65%)`,
          filter:'blur(2px)' }} />
      {/* Secondary */}
      <div className="absolute pointer-events-none rounded-full blur-[80px]"
        style={{ width:320, height:260, bottom:'-10%', right:'5%', background:c.c }} />
      {/* Category tag centred */}
      <div className="absolute inset-0 flex flex-col items-center justify-center gap-4">
        <div className="px-4 py-1.5 rounded-full text-[11px] font-bold uppercase tracking-[0.12em]"
        ...
        <svg width="240" height="60" viewBox="0 0 240 60" fill="none" opacity="0.18">
          ...
          <circle cx="120" cy="30" r="7" fill={color} opacity="0.6"/>
          <circle cx="160" cy="30" r="3" fill={color}/>
          <circle cx="210" cy="30" r="5" fill={color}/>
        </svg>
      </div>
    </div>
  )
...
// ── Prose renderer: paragraphs split by \n\n ──────────────────────────────
function Prose({ text }) {
  return (
    <div className="space-y-4">
      {text.split('\n\n').map((para, i) => (
        <p key={i} className="text-[1rem] leading-[1.88]" style={{ color:'var(--ink-mid)' }}>
          {para}
        </p>
      ))}
    </div>
...
// ── Related posts strip (other 2 posts in same or different categories) ───
function RelatedPosts({ current }) {
  const others = POSTS.filter(p => p.slug !== current.slug).slice(0, 2)
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-14">
      {others.map(p => (
        <Link key={p.slug} to={`/insights/${p.slug}`}
          ...
          <div className="flex-1 min-w-0">
            ...
            <h4 className="font-heading text-[0.9rem] font-bold text-ink leading-snug mb-1.5 line-clamp-2 group-hover:text-brand-purple transition-colors">
              {p.title}
            </h4>
  ...
  return (
    <div style={{ paddingTop: '68px', background: '#fff' }}>
      ...
      <section className="relative py-16 overflow-hidden"
        style={{ background: 'linear-gradient(160deg, #09090F 0%, #0A0C18 55%, #080E16 100%)', minHeight: 320 }}>
        {/* Colour bloom matching post accent */}
        <div className="absolute inset-0 pointer-events-none"
          style={{ background:`radial-gradient(ellipse 60% 65% at 65% 40%, ${post.color}1A 0%, ${post.color}08 40%, transparent 70%)` }} />
        <div className="absolute inset-0 pointer-events-none"
        ...
        <Container className="relative z-10">
          {/* Back link */}
          <Link to="/insights"
            className="post-hero-tag inline-flex items-center gap-2 mb-6 text-[0.82rem] font-semibold transition-all duration-200 hover:gap-3"
          ...
          <div className="post-hero-tag flex items-center gap-3 mb-5">
            <span className="text-[10.5px] font-bold uppercase tracking-[0.12em] px-3 py-1 rounded-full"
              style={{ color: post.color, background: post.bg.replace('0.06', '0.12'), border:`1px solid ${post.color}25` }}>
              {post.cat}
            </span>
          </div>

          {/* Title */}
          <h1 className="post-hero-h1 font-heading font-extrabold text-white tracking-[-0.03em] leading-[1.08] mb-6"
            style={{ fontSize: 'clamp(1.7rem, 3.5vw, 2.8rem)', maxWidth: 800 }}>
            {post.title}
          </h1>

          {/* Meta row */}
          <div className="post-hero-meta flex flex-wrap items-center gap-5">
            <div className="flex items-center gap-1.5 text-[0.82rem]" style={{ color:'rgba(255,255,255,0.4)' }}>
              <Calendar size={13} /> {post.date}
            </div>
            <div className="flex items-center gap-1.5 text-[0.82rem]" style={{ color:'rgba(255,255,255,0.4)' }}>
              <Clock size={13} /> {post.rt} read
            </div>
            <div className="flex items-center gap-2 ml-auto">
              <span className="text-[0.78rem]" style={{ color:'rgba(255,255,255,0.28)' }}>by Vyuhon Team</span>
            </div>
          </div>
        </Container>
      </section>
      ...
      <Container>
        <div className="max-w-[760px] mx-auto py-14">
          ...
          {/* Banner image */}
          <div className="post-banner mb-12">
            <PostBanner color={post.color} bg={post.bg} cat={post.cat} />
          </div>

          {/* Introduction */}
          <p className="text-[1.08rem] leading-[1.9] font-medium mb-10 pb-10 border-b"
            ...
            {post.sections.map((sec, i) => (
              <div key={i} className="section-block">
                ...
                <div className="flex items-start gap-3 mb-4">
                  ...
                  <h2 className="font-heading font-bold text-ink tracking-[-0.02em]"
          ...
          {/* Bottom CTA */}
          <div className="mt-16 p-7 rounded-[var(--r-xl)] relative overflow-hidden"
            style={{ background:'linear-gradient(135deg, #0A0A14, #0D0920)',
              border:`1px solid ${post.color}20` }}>
            <div className="absolute inset-0 pointer-events-none"
              style={{ background:`radial-gradient(ellipse 70% 70% at 100% 0%, ${post.color}12, transparent 65%)` }} />
            <div className="relative z-10">
              <p className="text-[0.85rem] font-bold uppercase tracking-[0.1em] mb-2" style={{ color: post.color }}>
                Interested in applying this at your organisation?
              </p>
              <h3 className="font-heading text-xl font-bold text-white mb-4 tracking-[-0.02em]">
                Talk to the Vyuhon engineering team
              </h3>
